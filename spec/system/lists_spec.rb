...
describe '一覧画面(todolists/index)のテスト' do
  before do
...
describe '詳細画面(todolists/show)のテスト' do
  before do
    visit todolist_path(list)
  end
  context '表示の確認'
    it '削除リンクが存在しているか' do
      expect(page).to have_link '削除', href: destroy_todolists_path(list.id)
    end
    it：'編集リンクが存在しているか' do
      expect(page).to have_link '編集', href: edit_todolists_path(list.id)
    end
...
describe '編集画面のテスト' do
  ...
  context '更新処理に関するテスト' do
    it '更新後のリダイレクト先は正しいか' do
      ...
      fill_in 'list[body]', with: Faker::Lorem.characters(number: 20)
      click_button '保存'
      expect(page).to have_curent_path todolist_path()
    end
  end
end